...
namespace App\Livewire\Welfare\Provisionals;

use Aaran\Welfare\Models\Project;
use Aaran\Welfare\Models\ProjectProduct;
use Aaran\Welfare\Models\ProjectProvisional;
...
class Index extends Component
{
    ...
    public function getSave(): string
    {
        if ($this->projects_id != '') {
            if ($this->vid == "") {
                ...
                $obj = ProjectProvisional::find($this->vid);
    ...
    {
        if ($id) {
            $obj = ProjectProvisional::find($id);
            $this->vid = $obj->id;
    ...
    {
        $this->sortField='id';
        return ProjectProvisional::search($this->searches)
    ...
    {
        return view('livewire.welfare.provisionals.index')->with([
            'list' => $this->getList()
        ]);
    }
    #endregion
